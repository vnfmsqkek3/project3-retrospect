...
```shell
curl --location --request POST "https://72qj78wnzi.execute-api.ap-northeast-2.amazonaws.com/produce" --header "Content-Type: application/json" \
--data-raw '{
    "input": 1
}'

curl -w "@curl-format.txt" --location --request POST "https://gz8m86ya1k.execute-api.ap-northeast-2.amazonaws.com/produce" --header "Content-Type: application/json" \
--data-raw '{
    "input": 1
}'
